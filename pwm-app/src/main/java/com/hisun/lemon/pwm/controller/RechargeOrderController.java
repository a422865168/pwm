...
public class RechargeOrderController<B> {
    ...
    public GenericDTO createOrder(@Validated @RequestBody GenericDTO<RechangeDTO> genRechangeDTO) {
		String ip="";
		RechangeDTO rechangeDTO=genRechangeDTO.getBody();
		if(StringUtils.isBlank(rechangeDTO.getPayerId())){
			rechangeDTO.setPayerId(genRechangeDTO.getUserId());
		}
		GenericDTO genericDTO=service.createOrder(rechangeDTO, ip);
		return genericDTO;
    }
