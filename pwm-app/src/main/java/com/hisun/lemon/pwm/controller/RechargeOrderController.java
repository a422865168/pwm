...
public class RechargeOrderController extends BaseController {
	...
	@ApiOperation(value = "充值下单1", notes = "生成充值订单，调用收银台")
	...
	public GenericRspDTO<RechargeRspDTO> createOrderMer(
			@Validated @RequestBody GenericDTO<RechargeDTO> genRechargeDTO) {
		RechargeRspDTO RechargeRspDTO = service.createOrderMer(genRechargeDTO);
		return GenericRspDTO.newSuccessInstance(RechargeRspDTO);
	}

	@ApiOperation(value = "商户充值结果查询", notes = "商户充值结果查询")
	@ApiImplicitParam(name = "x-lemon-usrid", value = "用户ID", paramType = "header")
	@ApiResponse(code = 200, message = "商户充值结果查询")
	@GetMapping(value = "/order/search/{orderNo}")
	public GenericRspDTO<OrderSearchRspDTO> orderSearch(@PathVariable("orderNo") String orderNo) {
		OrderSearchRspDTO orderSearchRspDTO = service.orderSearch(orderNo);
		return GenericRspDTO.newSuccessInstance(orderSearchRspDTO);
	}
	...
	public GenericRspDTO<TransferenceRspDTO> createTransference(
			@Validated @RequestBody GenericDTO<TransferenceReqDTO> genRechargeDTO) {
